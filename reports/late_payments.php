...
$sql = "SELECT c.customer_id, CONCAT(c.first_name, ' ', c.last_name) as customer_name, c.phone, c.num_late_payments, c.avg_days_late, COUNT(p.payment_id) as total_payments, SUM(CASE WHEN p.paid_date > p.due_date THEN 1 ELSE 0 END) as late_count FROM customers c JOIN payments p ON c.customer_id = p.customer_id GROUP BY c.customer_id HAVING late_count > 0 ORDER BY late_count DESC";
