...
<?php if ($query_num > 0) {
    ...
    switch($query_num) {
        case 1:
            ...
            $sql_display = "SELECT * FROM vehicles WHERE status = 'available' AND is_active = 1 ORDER BY make, model";
            $sql = $sql_display;
            break;

        case 2:
            $question = "Show me all our customers";
            $sql_display = "SELECT customer_id, first_name, last_name, phone, city, state FROM customers WHERE is_active = 1 ORDER BY last_name";
            $sql = $sql_display;
            break;

        case 3:
            $question = "Do we have any " . $make_input . " cars in stock?";
            $sql_display = "SELECT * FROM vehicles WHERE make = '[make]' AND status = 'available' AND is_active = 1";
            $sql = "SELECT * FROM vehicles WHERE make = '" . $make_input . "' AND status = 'available' AND is_active = 1";
            break;

        case 4:
            $question = "Who are our employees?";
            $sql_display = "SELECT * FROM employees WHERE is_active = 1 ORDER BY role, last_name";
            $sql = $sql_display;
            break;
        ...
        case 5:
            ...
            $sql = "SELECT COUNT(*) as cars_bought, SUM(price_paid) as total_spent FROM purchases WHERE MONTH(purchase_date) = MONTH(CURDATE()) AND YEAR(purchase_date) = YEAR(CURDATE()) AND is_active = 1";
            break;

        case 6:
            $question = "Which cars did we buy below book price?";
            $sql_display = "SELECT v.make, v.model, v.year, v.book_price, p.price_paid,\n       (v.book_price - p.price_paid) as savings\nFROM vehicles v\nJOIN purchases p ON v.vehicle_id = p.vehicle_id\nWHERE p.price_paid < v.book_price AND v.is_active = 1\nORDER BY savings DESC";
            $sql = "SELECT v.make, v.model, v.year, v.book_price, p.price_paid, (v.book_price - p.price_paid) as savings FROM vehicles v JOIN purchases p ON v.vehicle_id = p.vehicle_id WHERE p.price_paid < v.book_price AND v.is_active = 1 ORDER BY savings DESC";
            break;
        ...
        case 9:
            ...
            $sql = "SELECT CONCAT(e.first_name, ' ', e.last_name) as salesperson, COUNT(s.sale_id) as num_sales, SUM(s.sale_price) as total_revenue, SUM(s.commission) as total_commission FROM sales s JOIN employees e ON s.employee_id = e.employee_id WHERE s.is_active = 1 GROUP BY e.employee_id ORDER BY num_sales DESC";
            break;
        ...
        case 11:
            ...
            $sql = "SELECT v.make, v.model, v.year, SUM(r.est_cost) as estimated_total, SUM(r.actual_cost) as actual_total, (SUM(r.actual_cost) - SUM(r.est_cost)) as over_budget FROM repairs r JOIN purchases p ON r.purchase_id = p.purchase_id JOIN vehicles v ON p.vehicle_id = v.vehicle_id WHERE r.is_active = 1 GROUP BY p.purchase_id HAVING SUM(r.actual_cost) > SUM(r.est_cost) ORDER BY over_budget DESC";
            break;
        ...
        case 12:
            ...
            $sql = "SELECT CONCAT(c.first_name, ' ', c.last_name) as customer_name, c.phone, COUNT(s.sale_id) as cars_bought, SUM(s.sale_price) as total_spent FROM customers c JOIN sales s ON c.customer_id = s.customer_id WHERE s.is_active = 1 GROUP BY c.customer_id HAVING COUNT(s.sale_id) > 1 ORDER BY cars_bought DESC";
            break;
    }
